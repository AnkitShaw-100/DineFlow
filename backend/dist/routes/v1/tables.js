...
tablesRouter.get("/tables", requireAuthApi(), async (req, res, next) => {
    try {
        const status = z
        ...
        const tables = await Table.find(filter).sort({ code: 1 }).limit(200).lean();
        res.json({
            tables: tables.map((t) => ({
                id: t.code,
                seats: t.seats,
...
tablesRouter.post("/tables", requireAuthApi(), async (req, res, next) => {
    try {
        ...
        if (parsed.guests !== undefined)
            update.guests = parsed.guests;
        if (parsed.status === "seated")
            update.seatedAt = new Date();
        if (parsed.status === "available") {
            update.guests = undefined;
            update.seatedAt = undefined;
        }
        const doc = await Table.findOneAndUpdate({ code: parsed.code }, { $set: update, $setOnInsert: { code: parsed.code } }, { upsert: true, new: true }).lean();
