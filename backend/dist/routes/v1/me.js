...
function getClerkUserId(req) {
    const userId = req?.auth && typeof req.auth.userId === "string" ? req.auth.userId : null;
    if (!userId)
        throw new Error("Missing auth userId");
    return userId;
}
meRouter.get("/me", requireAuthApi(), async (req, res, next) => {
    try {
...
meRouter.post("/me/sync", requireAuthApi(), async (req, res, next) => {
    try {
        const clerkUserId = getClerkUserId(req);
        const clerkUser = await clerkClient.users.getUser(clerkUserId);
        const email = clerkUser.emailAddresses?.find((e) => e.id === clerkUser.primaryEmailAddressId)
            ?.emailAddress ?? clerkUser.emailAddresses?.[0]?.emailAddress;
        const doc = await User.findOneAndUpdate({ clerkUserId }, {
