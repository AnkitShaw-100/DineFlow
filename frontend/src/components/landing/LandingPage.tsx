  ...
  <header className="sticky top-0 z-40 border-b border-border/60 bg-background/80 backdrop-blur-xl">
    <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
      <Link to="/" className="flex items-center gap-2">
        ...
        <span className="font-display text-xl font-bold tracking-tight">Dine Flow</span>
      </Link>
    ...
    <div className="mx-auto grid max-w-7xl gap-12 px-6 pb-20 pt-16 md:grid-cols-2 md:items-center md:pt-24">
      ...
      >
        ...
        <p className="mt-6 max-w-lg text-lg leading-relaxed text-muted-foreground">
          Dine Flow is the modern POS that runs orders, tables, kitchen and payments in
          one calm, beautiful workspace — from the front of house to the back office.
        </p>
      ...
      >
        ...
        >
          ...
          <div>
            <div className="text-xs text-muted-foreground">New order · Table 12</div>
    ...
    <div className="mx-auto max-w-7xl px-6">
      <div className="mx-auto max-w-2xl text-center">
        ...
        <p className="mt-4 text-muted-foreground">
          A single workspace that replaces five disconnected tools — designed for service.
        </p>
      </div>
...
const HowItWorks = () => (
  <section id="how" className="bg-primary py-24 text-primary-foreground">
    <div className="mx-auto grid max-w-7xl gap-12 px-6 md:grid-cols-2 md:items-center">
      <div>
        <p className="text-sm font-semibold uppercase tracking-widest text-highlight">How it works</p>
        <h2 className="mt-3 font-display text-4xl font-bold md:text-5xl">
          From first tap to last receipt — under a minute.
        </h2>
        <p className="mt-4 text-primary-foreground/80">
          Set up your menu, invite your team, plug in a card reader. Dine Flow handles the rest.
        </p>
      ...
      <div className="relative">
        <div className="rounded-3xl bg-card p-6 text-card-foreground shadow-soft">
          ...
          <div className="mt-6 space-y-3">
            {[
              { t: "Table 4 · Burger combo", s: "In kitchen", c: "bg-highlight" },
              { t: "Table 9 · Pasta + wine", s: "Served", c: "bg-mint" },
              { t: "Table 12 · Tasting menu", s: "Paid", c: "bg-primary/15" },
            ].map((o) => (
              <div key={o.t} className="flex items-center justify-between rounded-xl border border-border p-3">
                <span className="text-sm font-medium">{o.t}</span>
                <span className={`rounded-full ${o.c} px-3 py-1 text-xs font-semibold text-primary`}>
                  {o.s}
                </span>
              </div>
            ))}
...
const plans = [
  {
    ...
    features: ["1 register", "Unlimited menu items", "Basic reports", "Email support"],
    highlight: false,
  },
  {
    name: "Pro",
    ...
    features: ["3 registers", "Kitchen display", "Online ordering", "Priority support"],
    highlight: true,
  },
  {
    name: "Enterprise",
    ...
    features: ["Unlimited registers", "Multi-location", "Custom integrations", "Dedicated CSM"],
    highlight: false,
  },
];
      ...
      <div className="mt-14 grid gap-6 md:grid-cols-3">
        {plans.map((p) => (
          ...
          >
            ...
            <p className={`mt-3 text-sm ${p.highlight ? "text-primary-foreground/80" : "text-muted-foreground"}`}>
              {p.desc}
            </p>
            <ul className="mt-6 flex-1 space-y-3 text-sm">
              {p.features.map((f) => (
                <li key={f} className="flex items-center gap-2">
                  <CheckCircle2 className={`h-4 w-4 ${p.highlight ? "text-highlight" : "text-primary"}`} />
                  {f}
                </li>
              ))}
            </ul>
...
const testimonials = [
  {
    quote: "We cut our average ticket time by 30% in the first month. The kitchen team actually smiles now.",
    name: "Maya Okafor",
    role: "Owner · Saffron Bistro",
  },
  {
    quote: "Switched from three tools to Dine Flow. Reports that took an hour now take ten seconds.",
    name: "Daniel Park",
    role: "GM · Hana Ramen",
  },
  {
    quote: "The floor plan and split-bill flow are the best we've ever used. Servers love it.",
    name: "Lucia Romero",
...
const Testimonials = () => (
  <section className="py-24">
    <div className="mx-auto max-w-7xl px-6">
      <div className="mx-auto max-w-2xl text-center">
        <p className="text-sm font-semibold uppercase tracking-widest text-primary">Loved by operators</p>
        <h2 className="mt-3 font-display text-4xl font-bold md:text-5xl">
  ...
  <section className="px-6 pb-24">
    <div className="mx-auto max-w-6xl overflow-hidden rounded-[2.5rem] bg-highlight px-8 py-16 text-center text-highlight-foreground shadow-soft md:px-16 md:py-20">
      ...
      <p className="mx-auto mt-4 max-w-xl text-highlight-foreground/80">
        Join 20,000+ restaurants running calmer, faster service. 30 days free, no card required.
      </p>
      <div className="mt-8 flex flex-wrap justify-center gap-3">
        ...
        <Button size="lg" variant="outline" className="border-highlight-foreground/30 bg-transparent">
          Talk to sales
        </Button>
      </div>
    </div>
    ...
    <div className="mx-auto grid max-w-7xl gap-8 px-6 md:grid-cols-5">
      <div className="md:col-span-2">
        ...
        <p className="mt-4 max-w-xs text-sm text-primary-foreground/70">
          The modern POS that helps restaurants serve calmer, faster and smarter.
        </p>
      </div>
      {[
        { title: "Product", links: ["Features", "Pricing", "Integrations", "Hardware"] },
