...
type TableStatus = "available" | "seated" | "billed" | "cleaning";
...
const statusMeta: Record<TableStatus, { label: string; bg: string; ring: string; dot: string }> = {
  available: {
    label: "Available",
    bg: "bg-card",
    ring: "ring-border",
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">Live floor plan — tap a table to manage it.</p>
        </div>
        <div className="flex gap-2">
          {(Object.keys(statusMeta) as TableStatus[]).map((s) => (
            ...
            >
              ...
              {statusMeta[s].label} <span className="text-muted-foreground">({counts[s] || 0})</span>
            </div>
          ))}
        </div>
      </div>
            ...
            return (
              ...
              >
                <div className="flex w-full items-center justify-between">
                  <span className="font-display text-2xl font-bold">{t.id}</span>
